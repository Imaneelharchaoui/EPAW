          ...
          <ul class="navbar-nav">
            ...
            <a class="nav-link text-white" href="<%= request.getContextPath()%>/HomeAdmin.jsp"><i class="fas fa-home"></i>  Home</a>
			 ...
			 <a class="navbar-brand " id="icon1" href="#" style="margin-left:650px;"><i class="fas fa-paw fa-xl" ></i>  E-paw</a>
      		 <form class="form-inline my-2 my-lg-0 ml-auto" method="post" action="<%= request.getContextPath()%>/RechercheAdmin">
		      <input class="form-control mr-sm-2" name="id" type="search" placeholder="Search by ID" aria-label="Search">
		      <select class="custom-select custom-select-sm mb-0" name="type" style="margin-right:7px;">
				  <option selected>TYPE</option>
				  <option value="Part">Particuliers</option>
				  <option value="Assoc">Associations</option>
				  <option value="Vet">Veterinaires</option>
				</select>
...
<form action="">
  ...
  <label class="btn btn-secondary" id="vet">
    ...
    <tr>
      ...
      <td colspan="3">
      	...
      	<form method ="post" action="<%= request.getContextPath() %>/BANNE">
      		...
      		<button type="submit" class="btn btn-danger" style="margin-right:10px;" ><i class="fas fa-user-minus"></i> Bannir</button>	
      		
      		<% } else { %>
      		<input id="id" name="id" type="hidden" value="<%= val1.get(val1.size()-1-i).id %>">
      		<input id="status" name="status" type="hidden" value="0">
      	    <button type="submit" class="btn btn-success" style="margin-right:10px;" ><i class="fas fa-user-plus"></i> Debannir</button>	
		  ...
		  <input type="hidden" name="status" value="<%= val1.get(val1.size()-1-i).status %>">	
		   <button type="submit" class="btn btn-warning"style="margin-right:10px;"  > <i class="fas fa-exclamation-triangle"></i> Avertir</button>	
		</form>
...
<table class="table table-sm table-dark" id="table1" style="display:none;">
  ...
  <%
    ...
    <tr>
      ...
      <td colspan="3"><%= val.get(val.size()-1-i).date %></td>
      <td colspan="3">
      	<div class="btn-group text-align" >
		...
		<form action="">
		   ...
		   <button type="button" class="btn btn-light" data-toggle="modal" data-target="#annonces<%=i %>" style="margin-right:10px;" ><i class="far fa-eye"></i> Annonces</button>	
				      ...
				      <div class="modal-body">
							  ...
							  <div class="card-body">
				    		   ...
				    		   <form method="post" action="<%= request.getContextPath() %>/Del">
				    		  	 <button  class="btn btn-info" type="button" data-toggle="modal" data-target="#ficheanimal<%= j%>" ><i class="far fa-eye"></i> Voir fiche</button>
				    		   		...
				    		   		<% if((val3.get(val3.size()-1-j).status).equals("0")){ %>  	
								      		...
								      		<input id="id" name="status" type="hidden" value="1">
								      		<button type="submit" class="btn btn-danger" style="margin-top:10px;"><i class="fas fa-trash" ></i> Supprimer</button>	
								      		
								      		<% } else { %>
								      		<input id="id" name="id" type="hidden" value="<%= val3.get(val3.size()-1-j).id %>">
								       		<input id="email" name="email" type="hidden" value="<%= val3.get(val3.size()-1-j).email %>">
								      		<input id="status" name="status" type="hidden" value="0">
								      	    <button type="submit" class="btn btn-success" style="margin-top:10px;"  ><i class="fas fa-trash-restore"></i> Restaurer</button>	
...
<div class="p-2">
<table class="table table-sm table-dark" id="table2" style="display:none;">
  <thead>
    <tr>
      <th scope="col">ID</th>
    ...
    <tr>
      ...
      <td colspan="6">
		...
		<form action="">
		   ...
		   <button type="button" class="btn btn-light" data-toggle="modal" data-target="#offres<%=i %>" style="margin-right:10px;" ><i class="far fa-eye"></i> Offres</button>	
				      ...
				      <div class="modal-body">
							  ...
							  <div class="card-body">
				    		   ...
				    		   <form method="post" action="<%= request.getContextPath() %>/DelOffre">
				    		  	 <button  class="btn btn-info" type="button" data-toggle="modal" data-target="#ficheoffre<%= j%>" ><i class="far fa-eye"></i> Voir fiche</button>
